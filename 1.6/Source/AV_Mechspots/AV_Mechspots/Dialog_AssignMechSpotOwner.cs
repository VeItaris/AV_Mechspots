    ...
    public class Dialog_AssignMechSpotOwner : Window
    {
        ...
        public void UpdateMechanitorlist()
        {
            ...
            for (int i = 0; i < tmpMaps.Count; i++)
            {
                ...
                tmpPawns.AddRange(tmpMaps[i].mapPawns.ColonySubhumansControllable);     // errors out for someone?! - probably because their rimworld version is pirated and not the current version?
            ...
            {
                if ((p.IsColonist || p.IsColonySubhumanPlayerControlled) && MechanitorUtility.IsMechanitor(p)) 
                {
